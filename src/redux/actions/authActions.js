...
export const UpdateUserApi = (postData, navigation, cb) => async dispatch => {
  ...
  cb && cb(true);
  http
    .post(`update-profile?id=115&branch_id=7`, postData, formDataHeader)
    .then(async response => {
...
export const ForgetPasswordApi =
  ...
  export const VerifyOtpApi = (postData, navigation, userId, cb) => async (
    dispatch,
    getState
  ) => {
    const userId = await AsyncStorage.getItem("@USER_ID");
    ...
    cb && cb(true);
    http
      .post(`customer/verify-login-otp`, postData, formDataHeader)
      .then(async response => {
        ...
        if (response.data && response.data.status === 200) {
          ...
          cb && cb(false);
          RNToasty.Info({
            title: response.data.message || response.data.massage,
            duration: 2,
          });
        }
      })
      .catch(error => {
        ...
        console.log('forget error : ', error);
        RNToasty.Error({
          title: error.response?.data?.massage || 'Error occurred',
          duration: 2,
        });
      });
  };
