...
const Otp = ({
  ...
  const handleChange = (name, value) => {
    console.log("otp handleChange data: ",name, value );
    setPostData({
      ...postData,
      [name]: value,
    });
  ...
  const handleSubmit = () => {
    if (postData.otp) {    
      ...
      VerifyOtpApi(postData, navigation, data => setLoading(data));
      setPostData({
        otp: null,
      });
    } else {
      ...
      <View style={globalStyles.justify_between}>
        <View style={globalStyles.center}>
          <View style={globalStyles.title_box}>
            {/* <Text style={{alignSelf:'center'}}>{route.params.data.otp}</Text> */}
